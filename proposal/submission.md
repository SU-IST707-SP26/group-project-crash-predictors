...
Our project addresses the needs of multiple stakeholder groups, each with specific requirements for collision risk predictions.

**City Traffic Planners** allocate limited budgets across competing safety infrastructure needs (traffic signals, protected crosswalks, speed cameras). They require intersection-level predictions with high precision—false positives waste scarce resources on interventions at genuinely safe locations. Predictions must include temporal breakdown (time of day, day of week) since some intersections are only dangerous during specific periods, affecting intervention type.

**Emergency Medical Services** strategically position ambulances and medical personnel to minimize response times to serious collisions. They need predictions that update frequently (hourly or by shift) and highlight when and where severe injury crashes are most likely. Their priority is recall over precision—failing to position resources near an actual severe crash has life-or-death consequences, while occasionally over-deploying is acceptable.

**Navigation Applications** (Google Maps, Waze, cycling apps) could integrate collision risk predictions to route vulnerable road users away from dangerous intersections. They need granular risk scores (street-segment level) that update at least daily and distinguish between user types—an intersection dangerous for cyclists may be relatively safe for cars. Real-time or near-real-time updates are essential for millions of daily routing decisions.

**Policy Makers** need to justify safety interventions and evaluate whether implemented measures achieve intended effects. They require models interpretable enough for non-technical audiences that can establish baseline predictions before interventions and enable valid before-after comparisons. Predictions must be stratified by borough and neighborhood to ensure safety investments are distributed equitably across the city.

These diverse stakeholder needs inform our modeling approach and evaluation strategy, prioritizing location-specific, time-varying predictions with sufficient interpretability to guide infrastructure decisions while achieving appropriate precision-recall balance for resource allocation.
